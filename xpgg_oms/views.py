...
class SaltAPI(object):
    ...
    # 3、安装了supervisor但是没启动{'return': [{'192.168.100.170': {'unix:///var/run/supervisor/supervisor.sock': {'reason': 'such file', 'state': 'no'}}}]}
    # 4、正常获取结果的情况：
    # 启动{'return': [{'192.168.68.50-master': {'djangoproject.runserver': {'state': 'RUNNING', 'reason': 'pid 1233, uptime 1 day, 6:56:14'}}}]}
    ...
    try:
        if request.is_ajax():
            ...
            if request.GET.get('server_list_tag_key') == 'modal_search_minion_id':
                ...
                return JsonResponse(result)
            elif request.POST.get('server_list_tag_key') == 'app_update' and request.user.is_superuser:
                ...
                if obj.is_valid():
                    ...
                    result['result'] = json.loads(error_str)
                return JsonResponse(result)
            elif request.POST.get('server_list_tag_key') == 'app_delete' and request.user.is_superuser:
                ...
                try:
                    ...
                    app_group_exist = AppGroup.objects.filter(
                        app_group_members__regex=r'^%s$|^%s,|,%s$|,%s,' % (
                        app_name, app_name, app_name, app_name)).exists()
                    if app_group_exist:
                    ...
                    result['result'] = str(e)
                return JsonResponse(result)
            elif request.GET.get('server_list_tag_key') == 'check_svn':
                ...
                try:
                    ...
                    result['result'] = '检查出错'
                    return JsonResponse(result)
            elif request.GET.get('server_list_tag_key') == 'search_app_log':
                try:
                    app_name = request.GET.get("app_name")
                    ...
                    if data_list.has_next():
                        ...
                        next_page_number = 0
                    result = {'status': True, 'log_app_username': log_app_username, 'create_time': create_time,
                              'release_result': release_result, 'has_previous': data_list.has_previous(),
                              'previous_page_number': previous_page_number, 'number': data_list.number,
                              'num_pages': data_list.paginator.num_pages, 'has_next': data_list.has_next(),
                              'next_page_number': next_page_number, 'log_content': log_content}
                    return JsonResponse(result)
                except Exception as e:
                    logger.error('查询日志错误了' + str(e))
                    return JsonResponse(result)
            elif request.POST.get('server_list_tag_key') == 'release_app':
                app_name = request.POST.get('app_name')
                ...
                try:
                    ...
                    for minion_id in minion_id_list:
                        ...
                        app_log.append(('-' * 10 + (
                                    'Minion_ID:%s开始发布 时间戳%s' % (minion_id, time.strftime('%X'))) + '-' * 10).center(
                            88) + '\n')
                                    ...
                                    if saltapi.get_token() is False:
                                        ...
                                        if app_svn_version:
                                            ...
                                            cmd_data = 'svn co -r %s %s  %s --username=%s --password=%s --non-interactive --no-auth-cache' % (
                                                release_svn_version, app_svn_url, app_svn_co_path, app_svn_user,
                                                app_svn_password)
                                        ...
                                        if response_data is False:
                                            ...
                                            if check_data in response_data:
                                                ...
                                                try:
                                                    app_svn_version = re.search(r'revision (\d+)\.',
                                    ...
                                    with requests.Session() as s:
                                        ...
                                        if saltapi.get_token() is False:
                                            ...
                                            jid = saltapi.async_state_api(tgt=minion_id, arg=["rsync_dir",
                                                                                              "pillar={'sync_file_method':'%s','source_path':'%s','name_path':'%s','user':'%s','sync_file_style':'%s'}" % (
                                                                                              sync_file_method,
                                            ...
                                            if jid is False:
                                                ...
                                                try:
                                                    ...
                                                    # 释放掉软连接
                                                    response_data = saltapi.file_remove_api(
                                    ...
                                    with requests.Session() as s:
                                        ...
                                        if saltapi.get_token() is False:
                                            ...
                                            if response_data is False:
                                                ...
                                                try:
                                                    response_data = response_data['return'][0][minion_id].rsplit(
                                                        split_cmd, 1)
                                                    ...
                                                    if isinstance(response_data, list):
                                                        for pid in response_data:
                                                            ...
                                                            if response_data is False:
                                                                ...
                                                                cmdline_result = ' '.join(
                                                                    response_data['return'][0][minion_id]['cmdline'])
                                    ...
                                    with requests.Session() as s:
                                        ...
                                        if saltapi.get_token() is False:
                                            ...
                                            if response_data is False:
                                                ...
                                                response_data = saltapi.supervisord_status_api(tgt=minion_id,
                                                                                               arg=[stop_cmd])
                                                # 当调用api失败的时候会返回false
                                                if response_data is False:
                                                    ...
                                                    try:
                                                        status_result = response_data['return'][0][minion_id][stop_cmd][
                                                            'state']
                                ...
                                with requests.Session() as s:
                                    ...
                                    if saltapi.get_token() is False:
                                        ...
                                        if response_data is False:
                                            ...
                                            try:
                                                response_data = response_data['return'][0][minion_id].rsplit(split_cmd,
                                                                                                             1)
                                                # 发现有的命令没有输出那么最终只会有成功失败的0、1返回这时候列表长度就=1
                                                if len(response_data) == 1:
                                                    if response_data[0] == '0':
                        ...
                        app_log.append(('-' * 10 + (
                                    'Minion_ID:%s发布完成 时间戳%s' % (minion_id, time.strftime('%X'))) + '-' * 10).center(
                            88) + '\n\n\n\n\n\n')
                    result['status'] = True
                    result['result'] = app_log
                    return JsonResponse(result)
                    ...
                    if result['status']:
                        ...
                        if 'SVN更新' in operation_content:
                            AppRelease.objects.filter(app_name=app_name).update(
                                app_svn_version_success=app_svn_version_success)
                    else:
                        release_result = '发布失败'
                    username = request.user.username
                    AppReleaseLog.objects.create(app_name=app_name, log_content=app_log, release_result=release_result,
                                                 username=username)
                    AppRelease.objects.filter(app_name=app_name).update(update_time=time.strftime('%Y年%m月%d日 %X'))
            elif request.POST.get('server_list_tag_key') == 'app_backup':
                app_name = request.POST.get('app_name')
                try:
                    app_backup_path = AppRelease.objects.get(app_name=app_name).app_backup_path
                    app_path = AppRelease.objects.get(app_name=app_name).app_path
                        ...
                        if saltapi.get_token() is False:
                            ...
                            for minion in minion_id_list:
                                ...
                                if response_data is False:
                                    ...
                                    response_data = response_data['return'][0][minion]
                                    if response_data is True:
                                        response_data = saltapi.state_api(tgt=minion_id, arg=["copy_dir",
                                                                                              "pillar={'source_path':'%s','name_path':'%s'}" % (
                                                                                              app_path,
                                                                                              app_backup_path),
                                                                                              "queue=True"])
                                        if response_data is False:
                    ...
                    AppReleaseLog.objects.create(app_name=app_name, log_content=app_log, release_result=release_result,
                                                 username=username)
                    AppRelease.objects.filter(app_name=app_name).update(update_time=time.strftime('%Y年%m月%d日 %X'))
            elif request.POST.get('server_list_tag_key') == 'app_restore':
                app_name = request.POST.get('app_name')
                try:
                    app_backup_path = AppRelease.objects.get(app_name=app_name).app_backup_path
                    app_path = AppRelease.objects.get(app_name=app_name).app_path
